...
MyPaint::MyPaint(QWidget *parent)
    : QWidget(parent), myBoard(":/map1.txt"),
      ...
      superButtonMap(":/SuperButton.png"),
	 ...
	 superButtonBrush.setTexture(superButtonMap);
}
...
void MyPaint::paintEvent(QPaintEvent *) {
	...
	//code for displaying map, only runs if map reads correctly
        if(!myBoard.readFail()) {
            ...
            //if waves are going, have turrets attack enemies and draw/animate the enemies
             if(myBoard.isMoving()){
                ...
                if (!myBoard.isWaveDone(myBoard.getNumSpawned()) && clock%20 == 1) {
                    if (myBoard.getNumSpawned()%8==0){
                        myBoard.addEnemy('h'); //spawn heavy
                    }
                    else if ((myBoard.getNumSpawned())%7==0) {
                        myBoard.addEnemy('s'); //spawn speedy
                    }
                    else if ((myBoard.getNumSpawned())%9==0) {
                        myBoard.addEnemy('o'); //spawn puny
                    }
                    else {
                        myBoard.addEnemy('p'); //spawn boss
                    }
                    myBoard.nextSpawned();
#ifdef DEBUG
                    ...
                    for(int j = 0; j < numEnemies; j++) {
                        ...
                        if(tempTower->isInRange(tempEnemy) && !tempEnemy->isDead() && clock%tempTower->getFiringRate()==0) {
                            ...
                            case 'f': painter.setPen(Qt::red); //fire tower
                                        break;
                            case 's': painter.setPen(Qt::magenta); //fire tower
                                        break;
                ...
                for(int i = 0; i < myBoard.enemyListSize(); i++) {
                    ...
                    if(!temp->isDead()) {
                        ...
                        else if(temp->getEnemyType() == 'o') {
                            painter.setBrush(QColor("magenta").darker(fadingFactor));//boss type
                        }
                        else {
                            painter.setBrush(Qt::cyan); //unknown type
                        }
            ...
            painter.drawRect(7*cellDim, cellDim*(numRows + 1), 2*cellDim, 2*cellDim);
            ...
            painter.drawRect(10*cellDim, cellDim*(numRows+1), 2*cellDim, 2*cellDim);
            ...
            painter.drawRect(7*cellDim, cellDim*(numRows+4), 2*cellDim, 2*cellDim);
            
	    painter.setBrush(superButtonBrush); //CHANGE TO IMAGE
            //highlight button if selected
            if(myBoard.isSuperTowerButtonClicked()){
                painter.setPen(Qt::red);
            }
            else{
                painter.setPen(Qt::black);
            }
            painter.drawRect(10*cellDim, cellDim*(numRows+4), 2*cellDim, 2*cellDim);
...
void MyPaint::mousePressEvent(QMouseEvent *e) {
    ...
    else if(onSuperTowerButton(e->x(), e->y())){ //Fire Tower construction button is clicked
        myBoard.superTowerClick(); //indicate button has been clicked
        myBoard.setTowerClicked(-1); //deselect any towers
    }
    ...
    else if(myBoard.isSuperTowerButtonClicked()) { //Fire Tower construction button is active (placing a Fire Tower)

        if(clickX >= 0 && clickX < numCols && clickY >= 0 && clickY < numRows && myBoard.getCell(clickY, clickX) == 'X'){
            myBoard.addTower(new Super(clickX, clickY)); //add fire tower to board
        }
        myBoard.setTowerClicked(-1); //deselect current tower, if any
    }
...
int MyPaint::onBasicTowerButton(int x, int y) {  // returns whether inputted x and y grid coordinates are in the Basic Tower button
    ...
    //checks to see if mouse press is within bounds of the button
    if(x >= 7*cellDim && x <= 9*cellDim && y>= cellDim*(numRows+1) && y <= cellDim*(numRows+3) ) {
...
int MyPaint::onQuickTowerButton(int x, int y) { //returns whether inputted x and y grid coordinates are in the Quick Tower button
    ...
    if( x >= 10*cellDim && x<=12*cellDim && y >= cellDim*(numRows+1) && y <= cellDim*(numRows+3) ) {
        return 1;
    }
    else{
        return 0;
...
int MyPaint::onFireTowerButton(int x, int y) { //returns whether inputted x and y grid coordinates are in Fire Tower button
    ...
    //check to see if x and y are within bounds of the button
    if( x >= 7*cellDim && x<=9*cellDim && y >= cellDim*(numRows+4) && y <= cellDim*(numRows+6)) {
        return 1;
    }
    else{
        return 0;
...
int MyPaint::onSuperTowerButton(int x, int y) { //returns whether inputted x and y grid coordinates are in Fire Tower button

    //check to see if x and y are within bounds of the button
    if( x >= 10*cellDim && x<=12*cellDim && y >= cellDim*(numRows+4) && y <= cellDim*(numRows+6)) {
        return 1;
    }
    else{
        return 0;
